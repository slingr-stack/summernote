  ...
  const ui = function(editorOptions) {
    return {
      ...
      button: function($node, options) {
        return renderer.create('<button type="button" class="note-btn btn btn-outline-secondary btn-sm" tabindex="-1">', function($node2, options2) {
          if (options2 && options2.data && options2.data.toggle === "dropdown") {
            $node2.removeAttr("data-toggle");
            $node2.attr("data-bs-toggle", "dropdown");
          ...
          } else if (options2 && options2.tooltip) {
            $node2.attr({
              title: options2.tooltip,
              "aria-label": options2.tooltip
            }).tooltip({
