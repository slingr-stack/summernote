...
(function() {
  ...
  class Context {
    ...
    createInvokeHandlerAndUpdateState(namespace, value2) {
      return (event) => {
        this.createInvokeHandler(namespace, value2)(event);
  ...
  class Statusbar {
    ...
    initialize() {
      ...
      this.$statusbar.on("mousedown touchstart", (event) => {
        ...
        const onStatusbarMove = (event2) => {
          let originalEvent = event2.type == "mousemove" ? event2 : event2.originalEvent.touches[0];
  ...
  const dropdown = renderer.create('<ul class="note-dropdown-menu dropdown-menu"></ul>', function($node, options) {
    const markup = Array.isArray(options.items) ? options.items.map(function(item) {
      ...
      const dataValue = 'data-value="' + value2 + '"';
      const dataOption = option !== void 0 ? ' data-option="' + option + '"' : "";
  ...
  const dropdownCheck = renderer.create('<ul class="note-dropdown-menu dropdown-menu note-check"></ul>', function($node, options) {
    const markup = Array.isArray(options.items) ? options.items.map(function(item) {
      const value2 = typeof item === "string" ? item : item.value || "";
      const content = options.template ? options.template(item) : item;
      return '<li aria-label="' + item + '"><a href="#" data-value="' + value2 + '">' + icon(options.checkClassName) + " " + content + "</a></li>";
    }).join("") : options.items;
    $node.html(markup).attr({ "aria-label": options.title });
    if (options && options.codeviewKeepButton) {
      $node.addClass("note-codeview-keep");
  ...
  const dialog = renderer.create('<div class="modal note-modal" aria-hidden="false" tabindex="-1" role="dialog"></div>', function($node, options) {
    ...
    $node.html([
      ...
      options.title ? '<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close" aria-hidden="true">&times;</button><h4 class="modal-title">' + options.title + "</h4></div>" : "",
    ...
    return {
      ...
      palette: function($node, options) {
        return renderer.create('<div class="note-color-palette"></div>', function($node2, options2) {
          const contents = [];
          for (let row = 0, rowSize = options2.colors.length; row < rowSize; row++) {
            const eventName = options2.eventName;
            const colors = options2.colors[row];
            const colorsName = options2.colorsName[row];
            const buttons = [];
...
//# sourceMappingURL=summernote-bs3.js.map
