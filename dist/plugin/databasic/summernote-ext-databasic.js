...
(function (factory) {
  ...
  var DataBasicPlugin = function DataBasicPlugin(context) {
    var self = this;
    ...
    self.events = {
      ...
      'summernote.dialog.shown': function summernoteDialogShown() {
        self.hidePopover();
      }
    };
    self.initialize = function () {
      ...
      var body = '<div class="form-group row-fluid">' + '<label>' + lang.databasic.testLabel + '</label>' + '<input class="ext-databasic-test form-control" type="text" />' + '</div>';
    ...
    self.setContent = function ($node) {
      $node.html('<p contenteditable="false">' + self.icon + ' ' + lang.databasic.name + ': ' + $node.attr('data-test') + '</p>');
    };
    ...
    self.showDialog = function () {
      ...
      self.openDialog(info).then(function (dialogInfo) {
    ...
    self.openDialog = function (info) {
      return $.Deferred(function (deferred) {
        ...
        ui.onDialogShown(self.$dialog, function () {
          ...
          $saveBtn.text(info.node ? lang.databasic.edit : lang.databasic.insert).on('click', function (event) {
  ...
  $.extend(true, $.summernote, {
    ...
    options: {
      popover: {
        databasic: [['databasic', ['databasicDialog', 'databasicSize100', 'databasicSize50', 'databasicSize25']]]
      }
    },
